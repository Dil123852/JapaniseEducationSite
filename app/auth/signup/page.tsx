...
export default function SignupPage() {
  ...
  const onSubmit = async (data: SignupForm) => {
    ...
    try {
      // All signups are students - teachers are created in Supabase
      await signUp(data.email, data.password, 'student', data.fullName);
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-[24px] soft-shadow border border-[#E5E7EB] p-8 md:p-10 space-y-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            {/* Full Name */}
            <div>
              <label htmlFor="fullName" className="block text-sm font-[500] text-[#2B2B2B] mb-2">
                {t.fullName}
              </label>
              <input
                {...register('fullName')}
              ...
              />
              {errors.fullName && (
                <p className="mt-1 text-sm text-[#EF6161]">{errors.fullName.message}</p>
              )}
            </div>
            ...
            {/* Confirm Password */}
            <div>
              <label htmlFor="confirmPassword" className="block text-sm font-[500] text-[#2B2B2B] mb-2">
                {t.confirmPassword}
              </label>
              <input
              ...
              {errors.confirmPassword && (
                <p className="mt-1 text-sm text-[#EF6161]">{errors.confirmPassword.message}</p>
              )}
            </div>
          ...
          <div className="text-center pt-4 border-t border-[#E5E7EB]">
            <p className="text-sm text-[#9CA3AF]">
              ...
              <Link href="/auth/login" className="text-[#2B2B2B] font-[500] hover:text-[#C2E2F5] transition-colors">
                {t.login}
              </Link>
            </p>
          </div>

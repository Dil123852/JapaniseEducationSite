...
import { getCoursePDFs, recordPDFDownload } from '@/app/lib/db/pdfs';
import Link from 'next/link';
import PDFListClient from './PDFListClient';

export default async function CoursePDFsPage({
  params,
}: {
  ...
  const pdfs = await getCoursePDFs(resolvedParams.id);
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
      <header className="bg-white shadow-sm border-b border-slate-200 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <h1 className="text-xl font-light text-slate-800">PDF Materials</h1>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        {pdfs.length === 0 ? (
          ...
          <PDFListClient pdfs={pdfs} studentId={profile.id} />
        )}
      </main>
    </div>
  );
